...
import { MdOutlineDelete, MdModeEdit, MdAdd, MdOutlineSportsSoccer } from 'react-icons/md';
  ...
  if (!isSignedIn) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-gray-900 text-white p-6">
        <Image src="/logo.png" alt="MCITYX Logo" width={64} height={64} className="mb-6 rounded-full" />
        <p className="mb-6 text-lg font-semibold">Please sign in to view your profile.</p>
        <Link
          href="/sign-in"
      ...
      {loading ? (
        ...
        paginatedLineups.map((lineup, index) => (
          ...
          >
            ...
            Created: {new Date(lineup.created_at).toLocaleString('en-US', {
  hour: 'numeric',
  minute: 'numeric',
  hour12: true,
  year: 'numeric',
...
<section className="max-w-5xl mx-auto mt-5 mb-8 rounded-xl shadow px-6 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-white dark:bg-gray-800 border border-sky-300 dark:border-sky-700 transition-colors duration-500">
  ...
  function handleDelete(id) {
    if (!confirm('Are you sure you want to delete this lineup?')) return;
    setDeletingId(id);
    fetch(`/api/lineups/${id}`, { method: 'DELETE' })
      .then((res) => {
        if (!res.ok) throw new Error('Failed to delete');
        setLineups((prev) => prev.filter((l) => l.id !== id));
      })
      .catch(() => alert('Error deleting lineup'))
