...
const Schedule = () => {
  ...
  const fetchApiMatches = useCallback(async (forceRefresh = false) => {
  ...
  useEffect(() => {
    if ((selectedTab === "prem" || selectedTab === "champ") && !apiDataFetched.prem && !apiDataFetched.champ) {
      fetchApiMatches();
    }
  ...
  const isCurrentTabLoading = useMemo(() => {
    return selectedTab === "prem" ? apiLoading.prem : 
           selectedTab === "champ" ? apiLoading.champ : 
  ...
  const renderEmptyState = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4">
        ...
        >
          <RotateCcw className={`w-4 h-4 mr-2 ${isCurrentTabLoading ? 'animate-spin' : ''}`} />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center gap-2 sm:gap-6 mb-8 overflow-x-auto pb-2">
          {tabs.map(({ key, label, img, color, description }) => {
            ...
            const isLoading = key === "prem" ? apiLoading.prem : 
                             key === "champ" ? apiLoading.champ : false;
            const hasData = key === "fifa" ? manualMatches.length > 0 : 
                           key === "prem" ? categorizedMatches.premier.length > 0 :
                           categorizedMatches.champions.length > 0;
            const hasError = key === "prem" ? apiError.prem :
                            key === "champ" ? apiError.champ : false;
        ...
        {(apiError.prem || apiError.champ) && (selectedTab === "prem" || selectedTab === "champ") && (
